...
The other variables `city`, `address`, `longitude` and `latitude` have to be selected by first accessing the respective column that is itself a data frame and then accessing the respective column inside that data frame. The resulting vectors are saved as columns in `venue_data`. `venue_data` has `r nrow(venue_data)` rows and `r ncol(venue_data)` columns. 
