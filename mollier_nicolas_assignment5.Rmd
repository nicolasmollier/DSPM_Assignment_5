...
The other variables `ity`, `address`, `longitude` and `latitude` have to be selected by first accessing the respective column that is itself a data frame and then accessing the respective column inside that data frame. The resulting vectors are saved as columns in `venue_data`. `venue_data` has `r nrow(venue_data)` rows and `r ncol(venue_data)` columns. 
...
# Task 6) Event locations in Great Britain

We repeat everything that we have done previously for German venues but set the `country_code` to "GB" this time to retrieve venues in GB. Another thing that we have to adapt are the extreme points to filter out those venues with location coordinates that are way outside the country borders of GB Otherwise, the code is not changed compared to the German case. 
...
# max_lat_fr <- 51.083333
# min_lat_fr <- 42.333333
# max_long_fr <- 8.216667
# min_long_fr <- -4.783333
...
ggplot() +
  geom_polygon(
    aes(x = long, y = lat, group = group), data = map_data("world", region = "UK"),
    fill = "gray90",color = "black") +
  geom_point(data = venue_data_complete, aes(longitude, latitude), 
             color = "royalblue3",
             alpha = 0.5) +
  theme_void() + coord_quickmap() +
  labs(title = paste("Event locations across Great Britain"), caption = "Source: ticketmaster.com") +
  theme(title = element_text(size=12, face='bold'),
    plot.caption = element_text(face = "italic")) 
```
